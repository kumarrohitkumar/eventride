...
/**
 * Session + client wiring for the guest app.
 *
...
const TOKEN_KEY = 'eventride.guest.token'
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (principal.ok) {
          const me = (await principal.json()) as { userId: string; role: 'GUEST'; guestId?: string }
          setSession({
            token,
            role: me.role,
            profile: { id: me.userId, name: '', guestId: me.guestId },
          })
        } else {
        ...
        setSession({ token, role: 'GUEST', profile: { id: '', name: '' } })
      }
      setRestoring(false)
    })()
  }, [])
